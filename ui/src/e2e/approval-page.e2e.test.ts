...
suite.define(() => {
  ...
  it("keeps one canonical winner across conflicting surfaces and terminal reload", async () => {
    ...
    try {
      ...
      expect(await mobile.page.title()).toBe("Command approval — BenchAGI Aurelius Vault");
      ...
      expect(await mobile.page.title()).toBe("Approved here — BenchAGI Aurelius Vault");
      ...
      expect(await mobile.page.title()).toBe("Approved — BenchAGI Aurelius Vault");
      expect(new URL(mobile.page.url()).pathname).toBe(approvalPath(""));
