...
describe("WorktreesPage lifecycle", () => {
  ...
  it.each(["refs/remotes/origin/main", undefined])(
    "creates automatically unless a base is selected (%s)",
    async (defaultBranch) => {
      const request = vi.fn((method: string) => {
      ...
      await waitForFast(() => expect(page.createBranches).toEqual(["refs/heads/main"]));
      expect(page.createBaseRef).toBe("");
      await page.createWorktree();
      expect(request).toHaveBeenCalledWith("worktrees.create", { repoRoot: "/tmp/repo" });
      page.createBaseRef = "refs/heads/main";
      await page.createWorktree();
      expect(request).toHaveBeenCalledWith("worktrees.create", {
        repoRoot: "/tmp/repo",
