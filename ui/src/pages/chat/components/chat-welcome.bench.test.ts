...
describe("chat welcome Bench identity", () => {
  ...
  it("reads the welcome agent from the session key, then the host default", () => {
    const fromSession = mountWelcome("agent:cole:main");
    expect(
      fromSession.querySelector<HTMLImageElement>(".agent-chat__welcome-portrait")?.src,
    ).toContain("agent-art/zig.png");
    const fromHost = mountWelcome("main", null, { assistantAgentId: "cole" });
    expect(
      fromHost.querySelector<HTMLImageElement>(".agent-chat__welcome-portrait")?.src,
    ).toContain("agent-art/zig.png");
  });
