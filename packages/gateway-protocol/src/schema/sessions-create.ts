...
export const SessionsCreateParamsSchema = closedObject({
  ...
  // Bench fork (#71): accept the reasoning toggle at create time so a caller with only
  // operator.write (e.g. a flyway seat) can set it without the operator.admin scope that
  // sessions.patch requires. Validated like the patch.
