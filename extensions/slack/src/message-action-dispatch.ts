...
/**
 * Bench fork #94: requester identity is only trusted when the current
 * conversation is a Slack conversation on the same account; sender ids from
 * other providers or Slack accounts must not be treated as workspace users of
 * the target account.
 */
...
export async function handleSlackMessageAction(params: {
  ...
  // from a caller-supplied tool context (Bench fork #94).
