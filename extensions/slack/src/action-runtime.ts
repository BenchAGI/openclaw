...
/**
 * Bench fork #94: end-user read authority attached by the message-action
 * dispatcher from server-injected identity. Absent means operator (config
 * policy only); "unverified" fails closed to current-conversation reads.
 */
...
/**
 * Bench fork #94: second read gate after config policy. Bind read access to
 * the requesting Slack end user. The bot token often sees many channels
 * (especially under groupPolicy "open"), so bot visibility alone must not let
 * one user read a channel they are not a member of.
 */
...
export async function handleSlackAction(
  ...
  const assertReadTargetAllowed = async (target: SlackActionChannelTarget) => {
    await assertSlackReadTargetAllowed({
      ...
      context,
      readOpts: buildActionOpts("read", target.teamId),
    });
  };
    ...
    switch (action) {
      ...
      case "downloadFile": {
        ...
        const downloaded = await slackActionRuntime.downloadSlackFile(fileId, {
          ...
          channelId,
          threadId: threadId ?? undefined,
          // Bench fork #94: end users need positive channel evidence on the file.
