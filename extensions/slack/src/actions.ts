...
/**
 * Bench fork #94: early-exit membership probe; intentionally not the monitor
 * allowlist fetch (monitor/auth.ts fetchSlackChannelMemberIds), whose TTL cache
 * is bound to the live monitor context that token-based actions do not have.
 */
...
function lacksSlackScopeProof(params: {
  ...
  threadId?: string;
  /**
   * Bench fork #94: require positive channel evidence on the file. A Slack
   * end user must not fetch a file the metadata cannot place in the channel.
   */
  ...
  const hasChannelEvidence = directIds.size > 0 || shares.length > 0;
