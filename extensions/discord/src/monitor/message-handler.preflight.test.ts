...
function createThreadBinding(
  overrides?: Partial<import("@benchagi/openclaw/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("@benchagi/openclaw/plugin-sdk/conversation-runtime").SessionBindingRecord;
}

function createPreflightArgs(params: {
  cfg: import("@benchagi/openclaw/plugin-sdk/config-runtime").OpenClawConfig;
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("@benchagi/openclaw/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("@benchagi/openclaw/plugin-sdk/config-runtime").OpenClawConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("drops guild message without mention when channel has configuredBinding and requireMention: true", async () => {
    const conversationRuntime = await import("@benchagi/openclaw/plugin-sdk/conversation-runtime");
