...
export async function pruneImportedSourceEntries(params: {
  ...
  for (const [syncKey, entry] of Object.entries(params.state.entries)) {
    ...
    // Durability guard (Bench fork): never delete a bridge page whose source
    // artifact points at a sibling user home. That defends against a state
    // rebuild adopting a satellite operator's mirror page and pruning it as
    // absent locally, while still allowing local non-home workspaces and
    // unsafe-local imports to prune.
