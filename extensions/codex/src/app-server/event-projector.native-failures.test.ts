...
describe("CodexAppServerEventProjector native tool failure recovery", () => {
  ...
  it("clears a git identity failure after the same commit succeeds (Bench fork #105)", async () => {
    ...
    await projector.handleNotification(
      forCurrentTurn("item/completed", {
        item: {
          ...
          status: "completed",
          commandActions: [],
          aggregatedOutput: "[fix/token abc1234] fix: refresh expired cowork seat tokens",
          exitCode: 0,
          durationMs: 1,
        },
      }),
