...
export class CodexToolProgressProjection {
  ...
  recordNativeToolError(params: {
    ...
    // Bench fork #105: a mutating native failure is retained until the SAME action
    // succeeds. A `git commit` retried after configuring the missing identity
    // matches through the recovery fingerprint so the stale failure clears.
    if (!isFailure && this.lastNativeToolError?.actionFingerprint) {
      ...
      if (recoveredFingerprint === this.lastNativeToolError.actionFingerprint) {
        this.lastNativeToolError = undefined;
      }
    }
    const actionFingerprint = isFailure
      ? (nativeCommandRecoveryFingerprint(
          params.item,
    ...
    if (terminalResolution) {
      this.lastNativeToolError = terminalResolution.lastToolError
        ? {
            ...terminalResolution.lastToolError,
            ...(actionFingerprint && !terminalResolution.lastToolError.actionFingerprint
              ? { actionFingerprint }
              : {}),
          }
        : terminalResolution.lastToolError;
...
/** Bench fork #105: stable identity for a native action so a retry can be matched. */
