...
describe("memory cli", () => {
  ...
  function tier1EnabledConfig() {
    return {
      memory: { search: { query: { tier1: { enabled: true } } } },
    };
  }
  ...
  it("injects tier1 context and prints json when the flag is enabled", async () => {
    ...
    expect(secretRefsCall.targetIds).toContain("memory.search.remote.apiKey");
    expect(secretRefsCall.targetIds).not.toContain("memory.search.query.reranker.apiKey");
  ...
  it("adds tier1 reranker secret targets only when the reranker SecretRef is active", async () => {
    ...
    expect(secretRefsCall.commandName).toBe("memory tier1");
    expect(secretRefsCall.targetIds).toContain("memory.search.remote.apiKey");
    expect(secretRefsCall.targetIds).toContain("memory.search.query.reranker.apiKey");
    expect(secretRefsCall.optionalActivePaths).toStrictEqual(
      new Set(["memory.search.query.reranker.apiKey"]),
    );
    expect(search).toHaveBeenCalled();
    expect(close).toHaveBeenCalled();
  });
  ...
  it("uses effective memorySearch.enabled for inherited tier1 reranker SecretRefs", async () => {
    getRuntimeConfig.mockReturnValue({
      ...
      agents: {
        list: [{ id: "main", memory: { search: { enabled: true } } }],
      },
    });
    const close = vi.fn(async () => {});
    const search = vi.fn(async () => []);
