 ...
 */
function injectClaudeSettings(args: readonly string[], patch: Record<string, unknown>): string[] {
  const normalized: string[] = [];
  let merged = false;
  for (let i = 0; i < args.length; i += 1) {
    const arg = args[i] ?? "";
...
export function normalizeClaudeBackendConfig(
  ...
  const permissionMode = isOpenClawRequestedYolo(context)
    ...
    : undefined;
  // Bench fork #77/#88: ultracode (xhigh effort + standing dynamic-workflow
  // orchestration) is opt-in per claude-cli backend config and reachable only via
  // the `ultracode` session settings key, so inject `--settings '{"ultracode":true}'`.
  // Keep `undefined` intact so an unset resumeArgs still falls back to args at spawn time.
  ...
  return {
    ...config,
    args: applyUltracode(normalizeClaudeBackendArgs(config.args, permissionMode)),
    resumeArgs: applyUltracode(normalizeClaudeBackendArgs(config.resumeArgs, permissionMode)),
    output,
    liveSession:
