...
including after symlink resolution. **Glob patterns** (containing `*`, `?`, `{}`)
only match recognized bootstrap basenames (`AGENTS.md`, `SOUL.md`, `IDENTITY.md`,
`USER.md`, `BOOTSTRAP.md`, and `MEMORY.md`) — this prevents a broad glob from
sweeping in unrelated files. **Explicit literal paths** are operator-intentional
and may name any workspace file, so they bypass the basename allowlist — use them
to inject live operational context such as `memory/ACTIVE_TASKS.md` into every
agent turn. Files are read fresh each run, so edits flow into the next turn.
