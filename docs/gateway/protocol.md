...
<AccordionGroup>
  ...
  <Accordion title="Models and usage">
    ...
    - `memory.tap.snapshot` is registered by the `memory-core` plugin and requires `operator.read`. It returns schema version 1 with canonical memory/dreaming health checks, bounded dream and memory excerpts, and aggregate indexed-session evidence for a requested `since`/`until` window. The window defaults to 24 hours and cannot exceed 48 hours; `maxCandidates` is capped at 25 and `maxQuoteChars` at 240. Pass `kinds` as a non-empty subset of `dream`, `memory`, and `session` to control which evidence kinds may be returned; manual operator calls default to all three. Scope-independent health checks may still validate local artifact readability, but excluded evidence is never emitted or counted as a candidate. The memory check reports configured full-text-only search as `warn`, and reports `error` when no search path is configured; it does not perform a live embedding request. `provenance.examined` counts selected, in-window source items before safety filtering and deduplication. Results use virtual provenance references, hard-drop secret-bearing or raw-identity excerpts, and never return workspace paths, session filenames, or transcript rows.
