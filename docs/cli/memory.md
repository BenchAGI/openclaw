---
summary: "CLI reference for `openclaw memory` (status/index/reset/search/tier1/forget/promote/promote-file/promote-explain/rem-harness/rem-backfill/session-backfill)"
read_when:
...
The command honors `memory.search.query.tier1.enabled`. When Tier-1 is disabled,
...
Promote an external markdown file (for example a Claude Code seat memory) into
native agent memory under `memory/seat/` and reindex once.
