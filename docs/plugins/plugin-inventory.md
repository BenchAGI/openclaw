...
- **[memory-durability](/plugins/reference/memory-durability)** (`@openclaw/memory-durability`) - included in OpenClaw. Aurelius memory-plane durability watchdog: a queryable health tool plus a managed cron watcher that alerts Slack when memory copies diverge, CORE.md goes stale, the bus stalls, or the vault falls behind.
...
- **[session-digest](/plugins/reference/session-digest)** (`@openclaw/session-digest`) - included in OpenClaw. Emit a structured SessionDigest JSON line into the agent workspace when a gateway session ends, feeding workspace-local memory reconciliation.
