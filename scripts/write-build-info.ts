...
export type BuildInfo = {
  version: string | null;
  /**
   * Bench fork #85: the release identity is the git TAG (e.g. 2026.9.2-bench.1), not
   * package.json (which deliberately tracks the upstream base). `git describe` surfaces
   * the nearest tag + commits-ahead + short sha so a running build self-reports its
   * true lineage even though `version` stays at the upstream base.
   */
  release: string | null;
  commit: string | null;
  builtAt: string;
...
function resolveGitRelease(rootDir: string, execFileSyncImpl: ExecFileSync): string | null {
  try {
    const raw = execFileSyncImpl("git", ["describe", "--tags", "--always", "--dirty"], {
      cwd: rootDir,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"],
    }).toString();
    const release = raw.trim();
    return release || null;
  } catch {
    return null;
  }
