...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("@benchagi/openclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced @benchagi/openclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced @benchagi/openclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
