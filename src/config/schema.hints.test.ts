...
// Bench fork: instanceId scopes memory-wiki vaults and health across co-hosted instances.
const SECTIONS_WITHOUT_DOCS = ["$schema", "meta", "attachments", "instanceId"] as const;
...
describe("mapSensitivePaths", () => {
  ...
  it("main schema yields correct hints (samples)", () => {
    ...
    expect(hints["memory.search.query.reranker.apiKey"]?.sensitive).toBe(true);
    expect(hints["agents.entries.*.memory.search.query.reranker.apiKey"]?.sensitive).toBe(true);
