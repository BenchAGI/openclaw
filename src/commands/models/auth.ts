...
/** Bench fork #75: agent context plus whether the credential lands in the default store. */
async function resolveModelsAuthAgentContext(
  rawAgentId?: string | null,
): Promise<ResolvedModelsAuthAgentContext & { agentId: string }> {
  const config = await loadValidConfigOrThrow();
  const { agentId, agentDir } = await resolveModelsAuthAgent(rawAgentId, config);
  const workspaceDir =
    resolveAgentWorkspaceDir(config, agentId) ?? resolveDefaultAgentWorkspaceDir();
  return {
    config,
    ...
    agentDir,
    workspaceDir,
    isDefaultAgent: normalizeOptionalString(rawAgentId ?? undefined) === undefined,
  };
}
...
function resolveProviderAuthDefaultModel(params: {
  ...
  const starterModel = matched?.auth.find(
    (method) => method.kind === params.credentialKind && method.starterModel,
  )?.starterModel;
  return starterModel ? normalizeAgentModelRefForConfig(starterModel) : undefined;
}
...
export async function modelsAuthPasteTokenCommand(
  ...
  const { agentId, agentDir } = authContext;
