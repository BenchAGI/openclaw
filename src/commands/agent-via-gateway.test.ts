...
function createGatewayPairingScopeError() {
  const err = new Error("gateway closed (1008): pairing required: more scopes requested");
  err.name = "GatewayTransportError";
  return Object.assign(err, {
    kind: "closed",
    code: 1008,
    reason: "pairing required: more scopes requested",
    connectionDetails: {
...
describe("agentCliCommand", () => {
  ...
  it("surfaces the device-approve fix for a pairing/scope rejection (Bench fork)", async () => {
    await withTempStore(async () => {
