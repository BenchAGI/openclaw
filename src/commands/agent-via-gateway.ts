...
/**
 * Bench fork: a gateway "closed" with a pairing/scope rejection — e.g. the CLI device
 * holds operator.read but running an agent turn needs operator.write, and the scope
 * upgrade is pending approval. This is an authz problem no fallback can solve, so
 * surface the one-line fix instead of masking it.
 */
