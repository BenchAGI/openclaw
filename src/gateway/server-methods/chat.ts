...
export const chatHandlers: GatewayRequestHandlers = {
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    const { cfg, entry, storePath, canonicalKey: sessionKey, legacyKey } = sessionLoadResult;
    const selectedAgent = validateChatSelectedAgent({
      cfg,
      requestedSessionKey: rawSessionKey,
      agentId: requestedAgentId,
    ...
    if (canAttemptBenchCloudBridge(bridgeAttempt)) {
      ...
      try {
        ...
        if (turn.dispatch === "remote-brain") {
          // The chat-send abort controller for this runId is already registered
          // (activeRunAbort) earlier in the flow; the remote-brain turn adopts it
          // so chat.abort and timeout expiry cancel the cloud poll the same way
          // they cancel a local run.
          const remoteAbortSignal = activeRunAbort.controller.signal;
          const ackPayload = {
            runId: clientRunId,
