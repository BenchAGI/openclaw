  ...
  "sessions.create": async ({ req, params, respond, context, client, isWebchatConnect }) => {
    ...
    const created = await createSessionEntryWithTranscript(
      ...
      async ({ sessionEntries }) => {
        // Inherit the parent's runtime selection (model/levels) up front when the
        // child names no model, so an explicit thinkingLevel/reasoningLevel in the
        // patch below still wins over inherited values.
        ...
        const patchStore =
          Object.keys(inheritedSelection).length > 0
            ? {
                ...sessionEntries,
                [target.canonicalKey]: {
                  ...sessionEntries[target.canonicalKey],
                  ...inheritedSelection,
                },
              }
            : sessionEntries;
