...
export function startChatDispatch(params: StartChatDispatchParams): void {
  ...
  // Bench fork #82: set when the turn was answered by the Bench cloud remote brain.
  let remoteBrainHandled = false;
  ...
  const dispatch = replyDispatch
    .runAgentMediaTranscript(dispatchAdmission, () =>
      measureDiagnosticsTimelineSpan(
        ...
        async () => {
          ...
          // Bench fork #82: route eligible chat.send turns to the Bench cloud remote
          // brain. Default off; requires gateway.benchCloud + cloudAuth on the request.
          const bridgeAttempt = {
            config: resolveBenchCloudBridgeConfig(cfg),
    ...
    .then(async (dispatchResult) => {
      if (acceptedMessageInjection || remoteBrainHandled) {
        return;
      }
...
// Bench fork #82: cloud-brain bridge request helpers.
