...
function analyzeSourceModule(params: { filePath: string; source: string }): {
  ...
  for (const match of source.matchAll(STATIC_FROM_IMPORT_RE)) {
    ...
    if (specifier === "@benchagi/openclaw/plugin-sdk/core" && importsDefinePluginEntry(importClause)) {
  ...
  it("follows relative import edges while scanning guarded contract graphs", () => {
    expect(
      analyzeSourceModule({
        ...
        import { y } from "@benchagi/openclaw/plugin-sdk/testing";
      `,
  ...
  it("detects aliased definePluginEntry imports from core", () => {
    expect(
      analyzeSourceModule({
        ...
        source: `
          import { definePluginEntry as dpe } from "@benchagi/openclaw/plugin-sdk/core";
          import { somethingElse } from "@benchagi/openclaw/plugin-sdk/core";
        `,
      }).importsDefinePluginEntryFromCore,
    ).toBe(true);
  });
