...
const ALLOWED_PLUGIN_SDK_FIXTURE_IMPORTS = new Set([
  ...
  'src/plugins/loader.test.ts:configSchema: (require("@benchagi/openclaw/plugin-sdk").emptyPluginConfigSchema)(),',
  'src/plugins/loader.test.ts:const { onDiagnosticEvent } = require("@benchagi/openclaw/plugin-sdk");',
  // Intentional jiti alias regression test.
  'src/plugins/loader.git-path-regression.test.ts:`import { resolveOutboundSendDep } from "@benchagi/openclaw/plugin-sdk/infra-runtime";',
  'src/plugins/loader.git-path-regression.test.ts:          "@benchagi/openclaw/plugin-sdk/infra-runtime": ${JSON.stringify(copiedChannelRuntimeShim)},',
]);
...
function findLoaderFixtureSdkImports(): string[] {
  ...
  for (const file of LOADER_FIXTURE_TEST_FILES) {
    ...
    for (const line of source.split("\n")) {
      if (
        line.includes('require("@benchagi/openclaw/plugin-sdk') ||
        (line.includes("import ") && line.includes('"@benchagi/openclaw/plugin-sdk'))
      ) {
