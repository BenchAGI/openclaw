...
describe("secrets runtime provider and media surfaces", () => {
  ...
  it("resolves enabled Tier-1 reranker SecretRefs for global memory.search and agent overrides (Bench fork #63)", async () => {
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config: asConfig({
        memory: {
          search: {
    ...
    expect(snapshot.config.memory?.search?.query?.reranker?.apiKey).toBe("default-judge-key");
    expect(snapshot.config.agents?.list?.[1]?.memory?.search?.query?.reranker?.apiKey).toBe(
      "agent-judge-key",
    );
  });

  it("treats Tier-1 reranker SecretRefs as inactive when the reranker is disabled (Bench fork #63)", async () => {
    const snapshot = await prepareSecretsRuntimeSnapshot({
      config: asConfig({
        memory: {
          search: {
    ...
    expect(snapshot.config.memory?.search?.query?.reranker?.apiKey).toEqual({
      source: "env",
      provider: "default",
