...
export function collectAgentMemorySearchAssignments(params: {
  ...
  const collectForAgent = ({ entry: rawAgent, source }: ListedAgentEntry) => {
    ...
    // Bench fork #63: Tier-1 reranker judge key. Active only when memory search,
    // Tier-1, and the reranker are all effectively enabled for this agent; an
    // agent that omits apiKey inherits the global memory.search key.
    const query = isRecord(memorySearch?.query) ? memorySearch.query : undefined;
    const reranker = isRecord(query?.reranker) ? query.reranker : undefined;
    const tier1 = isRecord(query?.tier1) ? query.tier1 : undefined;
    const rerankerActive =
      active &&
      (tier1?.enabled ?? defaultTier1?.enabled ?? false) === true &&
      (reranker?.enabled ?? defaultReranker?.enabled ?? false) === true;
    const hasRerankerApiKeyOverride = Boolean(reranker && Object.hasOwn(reranker, "apiKey"));
    const rerankerApiKeyTarget = hasRerankerApiKeyOverride ? reranker : defaultReranker;
    if (rerankerApiKeyTarget && Object.hasOwn(rerankerApiKeyTarget, "apiKey")) {
      collectRuntimeSecretInputAssignment({
        value: rerankerApiKeyTarget.apiKey,
        path: hasRerankerApiKeyOverride
          ? `${agentPath}.memory.search.query.reranker.apiKey`
          : "memory.search.query.reranker.apiKey",
        expected: "string",
        defaults: params.defaults,
        context: params.context,
        active: rerankerActive,
        inactiveReason:
          "agent, memory.search, memory.search.query.tier1, or memory.search.query.reranker is disabled.",
        owner,
        apply: (value) => {
          rerankerApiKeyTarget.apiKey = value;
        },
      });
      if (!hasRerankerApiKeyOverride && rerankerActive) {
        defaultRerankerApiKeyAssignmentCollected = true;
      }
    }
  ...
  if (
    defaultReranker &&
    Object.hasOwn(defaultReranker, "apiKey") &&
    !defaultRerankerApiKeyAssignmentCollected
  ) {
    collectRuntimeSecretInputAssignment({
      value: defaultReranker.apiKey,
      path: "memory.search.query.reranker.apiKey",
      expected: "string",
      defaults: params.defaults,
      context: params.context,
      active: false,
      inactiveReason:
        "no enabled agent inherits this Tier-1 reranker api key (memory search, tier1, or the reranker is disabled).",
      apply: (value) => {
        defaultReranker.apiKey = value;
      },
    });
  }
