...
/**
 * Bench fork #63: Tier-1 reranker judge keys resolve only while memory search,
 * Tier-1, and the reranker are effectively enabled for some agent. They are
 * deliberately NOT part of the static agent-runtime target set; the runtime
 * adds them as optional active paths so a disabled reranker never blocks a turn.
 */
export function getActiveMemoryRerankerSecretTargets(
  config: OpenClawConfig,
): { targetIds: Set<string>; optionalActivePaths: Set<string> } | undefined {
  const defaults = config.memory?.search as MemorySearchLike | undefined;
  const targetIds = new Set<string>();
  const optionalActivePaths = new Set<string>();
  const effective = (agent: MemorySearchLike | undefined) => ({
    searchEnabled: agent?.enabled ?? defaults?.enabled ?? true,
    tier1: agent?.query?.tier1?.enabled ?? defaults?.query?.tier1?.enabled ?? false,
    reranker: agent?.query?.reranker?.enabled ?? defaults?.query?.reranker?.enabled ?? false,
  });
  const isSecretRef = (value: unknown) =>
    Boolean(
      resolveSecretInputRef({ value, refValue: undefined, defaults: config.secrets?.defaults }).ref,
    );
  ...
  for (const agent of agents) {
    const reranker = agent.search?.query?.reranker;
    if (!reranker || !isSecretRef(reranker.apiKey) || agent.enabled === false) {
      continue;
    }
    const e = effective(agent.search);
    if (!(e.searchEnabled && e.tier1 && e.reranker)) {
      continue;
    }
    targetIds.add("agents.entries.*.memory.search.query.reranker.apiKey");
    optionalActivePaths.add(`${agent.path}.memory.search.query.reranker.apiKey`);
  }
  return optionalActivePaths.size > 0 ? { targetIds, optionalActivePaths } : undefined;
}
