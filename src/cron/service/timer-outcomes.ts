...
export function applyJobResult(
  ...
  if (result.status === "error") {
    ...
    // replies are terminal non-failures and keep the counter reset (Bench fork #100).
    const deliveryUnconfirmed =
      deliveryState.status === "not-delivered" && Boolean(deliveryState.deliveryFailedError);
      ...
      state.deps.log.warn(
        {
          jobId: job.id,
          jobName: job.name,
          deliveryError: deliveryState.deliveryFailedError,
