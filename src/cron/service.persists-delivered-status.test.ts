...
import type { CronDeliveryTrace, CronResolvedDeliveryState } from "./types.js";
...
function createIsolatedCronWithFinishedBarrier(params: {
  ...
  const cron = new CronService({
    ...
    runIsolatedAgentJob: vi.fn(async () => ({
      ...
      ...(params.deliveryState === undefined ? {} : { deliveryState: params.deliveryState }),
...
describe("CronService persists delivered status", () => {
  ...
  it("persists blocked-by-policy when the announce send was cancelled by a policy hook", async () => {
    // Bench fork #100 (delivery truth): a hook-cancelled send is terminal and
    // never reads as delivered, and it is not a transport-failure streak.
    let capturedEvent: { deliveryStatus?: string } | undefined;
    const reason = "cancelled_by_message_sending_hook: external disclosure requires approval";
    const updated = await runIsolatedJobAndReadState({
      ...
      onFinished: (evt) => {
        capturedEvent = evt as { deliveryStatus?: string };
      },
    });
    expectSuccessfulCronRun(updated);
    expect(updated?.state.lastDelivered).toBe(false);
    expect(updated?.state.lastDeliveryStatus).toBe("blocked-by-policy");
    expect(updated?.state.lastDeliveryError).toBe(reason);
  ...
  it("counts an unconfirmed bestEffort transport failure as a consecutive error while the run stays ok", async () => {
    // Bench fork #100: the job ran, but the delivery never got transport
    // confirmation. Run status stays ok while delivery state and the error
    // streak tell the truth.
    const updated = await runIsolatedJobAndReadState({
      job: buildAnnounceIsolatedAgentTurnJob("unconfirmed-send"),
      deliveryState: {
        status: "not-delivered",
        delivered: false,
        error: "telegram send failed: ETIMEDOUT",
  ...
  it("never reports delivered when only a failure marker arrives without an ack", async () => {
    const updated = await runIsolatedJobAndReadState({
      ...
      deliveryState: {
        status: "not-delivered",
        delivered: false,
        error: "provider error before ack",
