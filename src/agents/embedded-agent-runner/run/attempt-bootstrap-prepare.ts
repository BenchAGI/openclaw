...
export async function prepareEmbeddedAttemptBootstrap(params: {
  ...
  // F1 — Tier-1 retrieval-at-start (Bench fork #63). On a cold (full-bootstrap),
  // primary, non-heartbeat session start, prepend a small retrieved prior-context
  // slice ahead of MEMORY.md so a fresh thread about a prior topic is not cold.
  // The Tier-0 budget accounting above stays on the original `contextFiles`; the
  ...
  if (
    ...
    contextInjectionMode !== "never" &&
    !isHeartbeatLifecycleRunKind(attempt.bootstrapContextRunKind) &&
    isPrimaryBootstrapRun(attempt.sessionKey) &&
    attempt.config &&
    typeof attempt.prompt === "string" &&
    attempt.prompt.trim().length > 0
  ) {
    ...
    params.markStage("tier1-retrieval");
  }
