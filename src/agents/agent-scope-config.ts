...
export function resolveAgentEntry(cfg: OpenClawConfig, agentId: string): AgentEntry | undefined {
  const exact = resolveAgentEntryById(cfg, agentId);
  if (exact) {
    return exact;
  }
  // Bench fork: fall back to the persona label (identity.name) or a declared alias so
  // operators can address an agent by the name it presents.
  const requested = normalizeAgentId(agentId);
  return listAgentEntries(cfg).find((entry) => {
...
function resolveAgentEntryById(cfg: OpenClawConfig, agentId: string): AgentEntry | undefined {
