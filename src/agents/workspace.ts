...
export async function loadWorkspacePatternFilesWithDiagnostics(
  ...
  // Track provenance: paths discovered by expanding a glob must be a recognized
  // bootstrap basename (a broad glob could otherwise sweep in unrelated files),
  // but an explicitly-listed literal path is operator-intentional and may name
  // any workspace file (e.g. `memory/ACTIVE_TASKS.md`), so it bypasses the
  // basename allowlist.
