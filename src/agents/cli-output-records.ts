...
function isClaudeCliProvider(providerId: string): boolean {
  // Bench fork #80: the ultracode variant resolves to backend id "claude-cli-ultracode"
  // (and future variants may follow the same "claude-cli-<flavor>" convention). It is
  // the same Claude CLI speaking the same stream-json dialect, so recognize the whole
  // claude-cli-* family. Without this, those backends fail the dialect gate and the
  // JSONL parser drops native text deltas, result frames, AND tool events.
