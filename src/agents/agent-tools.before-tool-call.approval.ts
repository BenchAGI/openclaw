...
function resolvePluginToolApprovalTimeoutMs(approval: PluginApprovalRequest): number {
  // Bench fork #108: the ceiling keeps the wait inside the host turn's idle budget.
  // A silent wait longer than that budget does not risk the turn — it ends it,
  // while the system is correctly waiting for a person (observed 2026-08-01:
  // 119,977 ms against 60,000 ms).
  //
