...
export async function readLastGatewayErrorLine(
  ...
  // Bench fork #87: the LaunchAgent writes stderr to a real log file (not
  // /dev/null), so darwin reads stderr too — crash output stays diagnosable.
  const readStderr = true;
